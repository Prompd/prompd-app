...
/** SVG mask for the P logo — shared between gradient icon variants */
...
/**
 * GradientPrompdIcon — P icon with conic gradient that follows the mouse.
 * Uses direct DOM updates (zero re-renders). Listens on document-level mousemove.
 */
export function GradientPrompdIcon({ size = 18 }: { size?: number }) {
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      rafRef.current = requestAnimationFrame(() => {
        ...
        el.style.background = `conic-gradient(from ${deg}deg, #06b6d4, #8b5cf6, #ec4899, #f59e0b, #06b6d4)`
        el.style.transition = 'none'
      })
    }
    const onLeave = () => {
      if (!el) return
      el.style.background = 'linear-gradient(135deg, #06b6d4, #3b82f6)'
